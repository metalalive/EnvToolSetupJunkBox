...
- Directives prefixed with `proxy_ssl_` are for secure connection setup between nginx proxy server and origin backend server.
- In `proxy_pass`, you can also specify upstream label (e.g. `backend_app_345` in `http` block) , which passes the request to one of the sevrers in the `upstream` block.
  - supported in both `http` and `stream` blocks.

#### Reverse Proxy in HTTP block
...
- If `ssl` is specified in directive `listen`, nginx always ensures client requests are encrypted before passing them to origin server (using `proxy_pass`)
  - if not set, client should always send HTTP request to the nginx, otherwise nginx will return SSL error

#### Reverse Proxy in stream block
- If `ssl` is specified in directive `listen`
  - without secure connection setup (`proxy_ssl_`), [**SSL termination**](https://stackoverflow.com/a/39421271/9853105) will occur at nginx, the client packets will be transferred to origin backend server in [plaintext](https://en.wikipedia.org/wiki/Plaintext).
  - otherwise, nginx decrypts the client packets and **encrypts them again** using different certificate specified in `proxy_ssl_certificate` 
- If `ssl` is NOT specified in directive `listen`
  - all client packets will be simply passed to origin backend server **without modification**
  - the good thing is, nginx **will NOT** downgrade layer 7 protocol to HTTP/1.1
- If an origin server receives unencrypted packet, recheck whether the API endpoint in the server supports HTTP/1.1
  - otherwise error would happen, e.g. ALPN error, `server did not agree to a protocol` in `curl`
- Remind the TCP connection is maintained at **client** side and **origin server** side

#### Caching
- Use case : popular response, not frequently changed / modified
- supported ONLY in [`http` modules](https://nginx.org/en/docs/http/ngx_http_core_module.html), [`stream` module](https://nginx.org/en/docs/stream/ngx_stream_core_module.html) does **NOT** support caching
- `proxy_cache_path`
  - `keys_zone` specifies :
...
- [`sticky`](https://docs.nginx.com/nginx/admin-guide/load-balancer/http-load-balancer/#enabling-session-persistence) is available ONLY in Nginx Plus, not open source version
...
#### Layer 4 (TCP/UDP)
- [`upstream`](https://nginx.org/en/docs/stream/ngx_stream_upstream_module.html#upstream) directive defines a group of origin servers the nginx will pass client requests to.
  - both of `http` and `stream` blocks support `upstream` directive
  - algorithms applied to both should be the same (round-robin, `least_conn`, `least_time`)
  - the directive `server` in  [`upstream` in stream block](https://nginx.org/en/docs/stream/ngx_stream_upstream_module.html#upstream) has similar arguments as [`upstream` in http block](https://nginx.org/en/docs/http/ngx_http_upstream_module.html#upstream)
